...
% elements which are a part of the PML boundary of width n_layers:
% 
%     + + + + + +
%     + + + + + +     n_layers = 2
%     + + . . + +  
...
if (n_elemy <= n_layers*2) || (n_elemx <= n_layers*2) || n_layers == 0
    ...
    idx_plm = 1:1:n_elemx*n_layers;
    
    for indy = n_layers:1:n_elemy-n_layers-1
        % append indices of left column(s) for row indy
        idx_plm = [idx_plm, (n_elemx*indy+1:1:n_elemx*indy+n_layers)];
        % append indices of right column(s) for row indy
        idx_plm = [idx_plm, (n_elemx*(indy+1)-n_layers+1:1:n_elemx*(indy+1))];
    end
    % append all indices of top row(s)
    idxPmlElements = [idx_plm, (n_elems-n_elemx*n_layers+1:1:n_elems)]';
end