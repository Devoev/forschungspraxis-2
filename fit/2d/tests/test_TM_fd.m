...
%% Plot solution for magnetic field
figure(1)
idx2plot = 2*np+1:3*np;
[X,Y] = meshgrid(msh.xmesh, msh.ymesh);
h_surf = reshape(real(hbow(idx2plot)), [msh.nx, msh.ny]);
h_surf_plot = surf(X,Y,h_surf');
set(h_surf_plot,'LineStyle','none')
